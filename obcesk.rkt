...
(define (step s)
  (match s
    ...
    ((ev d ρ σ κ)
     (ko d σ κ))
    ((ko d σ `(,(begink es ρ) . ,κ))
     (eval-sequence es ρ σ κ))
    ((ko d σ `(,(ifk e1 e2 ρ) . ,κ))
     (let ((e (if d e1 e2)))
       (ev e ρ σ κ)))
    ((ko d σ `(,(letk x es ρ) . ,κ))
     (let* ((a (alloc))
            (ρ‘ (hash-set ρ x a))
            (σ‘ (hash-set σ a d)))
       (eval-sequence es ρ‘ σ‘ κ)))
    ((ko d σ `(,(letreck x es ρ) . ,κ))
     (let* ((a (hash-ref ρ x))
            (σ‘ (hash-set σ a d)))
       (eval-sequence es ρ σ‘ κ)))
    ((ko d σ `(,(setk x ρ) . ,κ))
    ...
    ((ko d σ `(,(ratork es ρ) . ,κ))
     (let* ((φ (randk d (cdr es) '() ρ))
            (κ‘ (cons φ κ)))
       (ev (car es) ρ σ (cons φ κ))))
    ((ko d0 σ `(,(randk d1 '() ds ρ) . ,κ))
     (let* ((ds‘ (cons d0 ds))
            (ds“ (reverse ds‘)))
       (apply-procedure d1 ds“ σ κ)))
    ((ko d0 σ `(,(randk d1 es ds ρ) . ,κ))
     (let* ((ds‘ (cons d0 ds))
            (φ (randk d1 (cdr es) ds‘ ρ))
            (κ‘ (cons φ κ)))
       (ev (car es) ρ σ (cons φ κ))))
    ((ko (addr a) σ `(,(membersetk x e ρ) . ,κ))
     (let* ((φ (membersetvaluek x a ρ))
            (κ‘ (cons φ κ)))
       (ev e ρ σ (cons φ κ))))
    ((ko d0 σ `(,(membersetvaluek x a ρ) . ,κ))
     (let* ((d1 (hash-ref σ a))
            (d1‘ (hash-set d1 x d0))
            (σ‘ (hash-set σ a d1‘)))
       (ko (addr a) σ‘ κ)))
    ((ko (addr a) σ `(,(membergetk x ρ) . ,κ))
     (let* ((d0 (hash-ref σ a))
            (d1 (hash-ref d0 x)))
       (ko d1 σ κ)))))
